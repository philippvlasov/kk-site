    ...
    $('#cart').on('click', function (e) {
        ...
        if ($activeInputs.length === 0) {
            ...
            $.each(result, function (i, val) {
                ...
                $.each(val, function () {
                    $modalBody.find('ul').append('<li id="' + this.id + '">' + '<label>' + '<input name="service[' + i + ']" type="checkbox" checked >' + '<span class="checkmark"></span>' + '<span class="item_name">' + this.innerHTML + '</span>' + '</label>' + '</li>');
                });
            });
            $('#contentModal').modal('show');
        }
